...
export class DatosgeneralesComponent implements OnInit {
  ...
  editar(){
    ...
    this.formModel.get('fechaNac').setValidators([]);
  ...
  cambiarPassword(){
    ...
    this.formModelPassword.get('newPassword').setValidators([Validators.required, Validators.pattern('(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&].{7,}'), Validators.maxLength(100)]);
    this.formModelPassword.get('newPassword').updateValueAndValidity();
    this.formModelPassword.get('newPassword2').setValidators([Validators.required, Validators.pattern('(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&].{7,}'), Validators.maxLength(100)]);
  ...
  guardarCambios(){
    this.utilsService.confirm('updateUsuario', constantes.MENSAJE_EDITAR_USUARIO, constantes.MENSAJE_GUARDAR_USUARIO_HEADER).subscribe(res => {
      ...
      if(this.usuario.fecha_nacimiento != null){
